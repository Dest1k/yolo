#!/usr/bin/env python3
"""
NanoDet-Plus (NCNN) sidecar — anchor-free FPN detector, strong on small objects,
CPU-real-time on Pi 5. Same stream/panel/manual-capture/gimbal scaffolding as the
YOLO-FastestV2 sidecar; only the model + decode differ.

Runs NanoDet-Plus (RangiLyu/nanodet) on NCNN and serves the same annotated MJPEG
stream + web panel as the other sidecars: manual drag-to-lock capture, IoU
tracking, SIYI gimbal. Open http://<board-ip>:8080. Heavier than YOLO-FastestV2
but with an FPN over 3-4 strides, so it detects small objects much better while
still running comfortably real-time on a Pi 5 CPU.

NanoDet-Plus is anchor-free: a GFL head with integral (DFL) box regression. The
ncnn export emits ONE output tensor [num_points, nc + 4*(reg_max+1)] concatenated
over the feature levels (strides 8/16/32/64). The sidecar decodes it (per point:
argmax class score, softmax-integral box distances → box). Decode params depend on
the export, so they're env-tunable — use --inspect first.

Config (YOLO_* shared with the other sidecars):
  ND_PARAM / ND_BIN     ncnn model files                              [required]
  ND_INPUT              square input size (match training)            (default 416)
  ND_STRIDES            FPN strides                                   (default 8,16,32,64)
  ND_REG_MAX            DFL bins per side − 1                         (default 7)
  ND_OUTPUT             head output blob name (from --inspect)        (default: last output)
  ND_INPUT_BLOB         model input blob name        (default: first input / "in0")
  ND_MEAN / ND_STD      BGR mean / std for normalisation   (default nanodet ImageNet)
  ND_THREADS            inference threads                             (default 4)
  ...
  --inspect     load the model, sweep input sizes, print output shape/points, exit.

See the README for: install (ncnn), training on your GPU, and tuning.
"""
...
# ── NanoDet-Plus NCNN inference + GFL/DFL decode ──────────────────────────────
class NanoDetNCNN:
    """NanoDet-Plus (RangiLyu) on NCNN — anchor-free GFL head with integral (DFL)
    box regression and an FPN over 3-4 strides, so it reads small objects far
    better than the 2-scale YOLO-FastestV2 while staying CPU-real-time.

    The ncnn export emits ONE output tensor [num_points, nc + 4*(reg_max+1)],
    concatenated over the feature levels (strides 8/16/32/64), row-major (y,x)
    within each level. Decode per point: argmax of the class scores (sigmoid is
    auto-applied if the export left them as logits), then each of the 4 box sides
    is a softmax-integral over (reg_max+1) bins → a distance from the cell centre
    (x*stride, y*stride) to that edge. Letterbox preprocessing with NanoDet's
    ImageNet BGR mean/std. Mirrors nanodet's demo_ncnn; every decode parameter is
    overridable and `--inspect` probes the real model so you can converge fast.
    """

    def __init__(self):
        import ncnn
        self.ncnn = ncnn
        self.input = int(env("ND_INPUT", "416"))
        self.reg_max = int(env("ND_REG_MAX", "7"))
        self.strides = [int(s) for s in env("ND_STRIDES", "8,16,32,64").split(",")]
        self.conf = float(env("YOLO_CONF", "0.4"))
        self.nms = float(env("YOLO_NMS", "0.45"))
        # NanoDet's normalisation (BGR order; substract_mean_normalize wants 1/std).
        self.mean = [float(x) for x in env("ND_MEAN", "103.53,116.28,123.675").split(",")]
        self.std = [float(x) for x in env("ND_STD", "57.375,57.12,58.395").split(",")]
        self._cls_sigmoid = None     # auto-detected: are class scores raw logits?
        self._priors_cache = None; self._priors_inp = None
        self._dbg = True             # print one decode diagnostic line
        ...
        if not param or not bin_:
            sys.stderr.write("ERROR: set ND_PARAM and ND_BIN to your nanodet .param/.bin\n")
            sys.exit(2)
        ...
        self.out_blob = env("ND_OUTPUT") or (outs[-1] if outs else "output")
    ...
    def _fail(self, blob, why):
        ...
        sys.stderr.write(
            f"NanoDet extract/decode failed for '{blob}' ({why}). Real outputs: "
            f"{self._names('output')}. ncnn -100 usually means ND_INPUT ({self.input}) "
            f"doesn't match the export — try --inspect / ND_INPUT=320|416.\n")

    def _priors(self):
        """Grid centre priors (x, y, stride) in nanodet order: strides ascending,
        y outer, x inner. Cached per input size."""
        if self._priors_cache is not None and self._priors_inp == self.input:
            return self._priors_cache
        pts = []
        for s in self.strides:
            fh = math.ceil(self.input / s); fw = math.ceil(self.input / s)
            for yy in range(fh):
                for xx in range(fw):
                    pts.append((xx, yy, s))
        self._priors_cache = pts; self._priors_inp = self.input
        return pts

    # NanoDet has no box/score-formula ambiguity, so there's nothing to autotune.
    def autotune(self, frames):
        print("  autotune: NanoDet has a fixed decode — nothing to tune.")

    def autoconfig(self):
        """Probe the single output once: pick a working blob and sanity-check that
        the point count matches the stride grid for ND_INPUT."""
        outs = self._names("output") or [self.out_blob]
        ok = None
        try:
            ex = self.net.create_extractor()
            d = self.ncnn.Mat(self.input, self.input, 3); d.fill(0.0)
            ex.input(self.in_name, d)
            for nm in outs:
                try:
                    ret, m = ex.extract(nm)
                except Exception:
                    continue
                if ret == 0:
                    ok = (nm, np.array(m)); break
        except Exception as e:
            print(f"  auto: probe failed ({e}) — set ND_INPUT / ND_OUTPUT (see --inspect)")
            return
        if not ok:
            print(f"  auto: no output extracted at input={self.input} — likely ND_INPUT "
                  f"mismatch; try --inspect (sweeps sizes).")
            return
        self.out_blob = ok[0]
        P = max(ok[1].shape); C = min(ok[1].shape)
        exp = sum(math.ceil(self.input / s) ** 2 for s in self.strides)
        nc = C - 4 * (self.reg_max + 1)
        print(f"  auto: output={self.out_blob} channels={C} ⇒ nc={nc} points={P} "
              f"(expected {exp} for strides {self.strides} @ {self.input})")
        if P != exp:
            print("  auto: ⚠️ point count ≠ grid — adjust ND_INPUT / ND_STRIDES / ND_REG_MAX.")
    ...
    def inspect(self):
        ...
        got = False
        for sz in ([self.input] + [s for s in (320, 416, 256, 512, 288, 384) if s != self.input]):
            try:
                ex = self.net.create_extractor()
                d = self.ncnn.Mat(sz, sz, 3); d.fill(0.0); ex.input(self.in_name, d)
            except Exception as e:
                print(f"  input={sz}: forward error {e}"); continue
            any_ok = False
            for nm in (outs or [self.out_blob]):
                try:
                    ret, m = ex.extract(nm)
                except Exception as e:
                    print(f"  input={sz} {nm}: ERROR {e}"); continue
                if ret != 0:
                    continue
                a = np.array(m); any_ok = True
                C = min(a.shape) if a.ndim == 2 else a.shape[-1]
                P = max(a.shape) if a.ndim == 2 else None
                nc = C - 4 * (self.reg_max + 1)
                exp = sum(math.ceil(sz / s) ** 2 for s in self.strides)
                flag = "" if P == exp else f"  ⚠️ ≠ expected {exp}"
                print(f"  input={sz} {nm}: shape {a.shape} ⇒ nc={nc} (reg_max={self.reg_max}) "
                      f"points={P}{flag}")
            if any_ok:
                got = True
                if sz == self.input:
                    break
        if not got:
            print("  No size extracted → CONVERSION/ncnn problem (re-export with onnxsim, "
                  "or use the trainer's export_ncnn.py which verifies the model).")
        else:
            print("\n  Pick the ND_INPUT whose points match (no ⚠️). Set ND_STRIDES/ND_REG_MAX "
                  "if your config differs from the nanodet-plus default (8,16,32,64 / 7).")
    ...
    def detect(self, bgr):
        ...
        mat = self.ncnn.Mat.from_pixels(
            np.ascontiguousarray(canvas), self.ncnn.Mat.PixelType.PIXEL_BGR, self.input, self.input)
        mat.substract_mean_normalize(self.mean, [1.0 / s for s in self.std])
        ex.input(self.in_name, mat)
        try:
            ret, m = ex.extract(self.out_blob)
        except Exception as e:
            ret, m = -1, None; self._fail(self.out_blob, e)
        if ret != 0 or m is None:
            self._fail(self.out_blob, f"ncnn code {ret}"); time.sleep(0.05); return []
        out = np.array(m)
        if out.ndim == 3:
            out = out.reshape(-1, out.shape[-1]) if out.shape[0] == 1 else out.squeeze()
        if out.ndim != 2:
            self._fail(self.out_blob, f"unexpected output ndim {out.ndim}"); return []
        if out.shape[0] < out.shape[1]:      # (C, P) → (P, C)
            out = np.ascontiguousarray(out.T)
        return self._nms(self._decode(out, r, dw, dh, ow, oh))

    def _letterbox(self, bgr):
        """Aspect-preserving resize to input×input, pad with 0 (nanodet resize_uniform)."""
        h, w = bgr.shape[:2]
        inp = self.input
        r = min(inp / w, inp / h)
        nw, nh = int(round(w * r)), int(round(h * r))
        resized = cv2.resize(bgr, (nw, nh))
        canvas = np.zeros((inp, inp, 3), dtype=np.uint8)
        dw, dh = (inp - nw) // 2, (inp - nh) // 2
        canvas[dh:dh + nh, dw:dw + nw] = resized
        return canvas, r, dw, dh

    def _decode(self, out, r, dw, dh, ow, oh):
        rm1 = self.reg_max + 1
        C = out.shape[1]
        nc = C - 4 * rm1
        if nc <= 0:
            self._fail(self.out_blob, f"channels {C} < 4*(reg_max+1)={4*rm1}; check ND_REG_MAX"); return []
        priors = self._priors()
        if len(priors) != out.shape[0]:
            self._fail(self.out_blob, f"points {out.shape[0]} ≠ grid {len(priors)}"); return []

        cls = out[:, :nc]
        if self._cls_sigmoid is None:
            self._cls_sigmoid = bool(cls.min() < -0.01 or cls.max() > 1.01)
        if self._cls_sigmoid:
            cls = 1.0 / (1.0 + np.exp(-cls))
        labels = cls.argmax(1); scores = cls.max(1)
        keep = np.where(scores >= self.conf)[0]
        if self._dbg:
            self._dbg = False
            sys.stderr.write(
                f"nanodet dbg: C={C} nc={nc} points={out.shape[0]} cls_sigmoid={self._cls_sigmoid} "
                f"max_score={float(scores.max()):.3f} #>={self.conf}:{int(keep.size)}\n")
        if keep.size == 0:
            return []

        reg = out[:, nc:nc + 4 * rm1].reshape(-1, 4, rm1)
        bins = np.arange(rm1, dtype=np.float32)
        res = []
        for i in keep:
            x, y, s = priors[i]
            rr = reg[i]                                  # (4, rm1) logits
            e = np.exp(rr - rr.max(axis=1, keepdims=True))
            sm = e / e.sum(axis=1, keepdims=True)
            d = (sm * bins).sum(axis=1) * s              # [left, top, right, bottom]
            ctx, cty = x * s, y * s
            x1 = (ctx - d[0] - dw) / r; y1 = (cty - d[1] - dh) / r
            x2 = (ctx + d[2] - dw) / r; y2 = (cty + d[3] - dh) / r
            x1 = min(max(x1, 0.0), ow); x2 = min(max(x2, 0.0), ow)
            y1 = min(max(y1, 0.0), oh); y2 = min(max(y2, 0.0), oh)
            res.append((x1, y1, x2, y2, float(scores[i]), int(labels[i])))
        return res
...
class RpicamCapture:
    ...
    def read(self):
        if self.proc is None:
            return False, None
        while True:
            # Most recent complete JPEG only (markers are byte-stuffed, so rfind is safe);
            # dropping older frames keeps latency low after any stall.
        ...
        def _stream(self):
            ...
            try:
                while state.running:
                    ...
                    if frame is None:
                        time.sleep(0.003); continue   # only send NEW frames (no dup-flood → low latency)
                    last_seq = seq
...
<title>NanoDet-Plus panel</title><style>
...
def main():
    ...
    print("NanoDet-Plus sidecar (NCNN / CPU)")
    detector = NanoDetNCNN()
    detector.autoconfig()                                # probe output blob + grid
    ...
    print(f"  input blob: {detector.in_name}   output blob: {detector.out_blob}")
    print("  (no detections? run with --inspect; check ND_INPUT / ND_STRIDES / ND_REG_MAX)")
