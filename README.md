...
Совет новичку: **настрой и проверь всё по USB на столе**, а на дрон для полётов
перенеси на UART. Ниже — оба варианта целиком, по отдельности.

---

<details>
<summary><b>🔌 Вариант A — по USB (без пайки). Развернуть пошагово</b></summary>
...
ND_PARAM=nanodet.param ND_BIN=nanodet.bin ND_INPUT=320 \
  ...
  python3 tools/nanodet-sidecar/nanodet_ncnn_sidecar.py
```
В консоли должна появиться жёлтая «BETAFLIGHT FOLLOW MODE ENABLED» и адрес
`http://<ip-пи>:8080`. Дальше — раздел **«Проверка БЕЗ пропеллеров»** ниже.

> Особенность USB: пока Pi держит порт, Configurator к этому полётнику по USB не
> подключишь. Поэтому направления каналов (шаг проверки) удобнее смотреть на
> UART-подключении или по реакции моторов со снятыми пропами.
...
<details>
<summary><b>🔧 Вариант B — по UART (пайка). Развернуть пошагово</b></summary>
...
ND_PARAM=nanodet.param ND_BIN=nanodet.bin ND_INPUT=320 \
  ...
  python3 tools/nanodet-sidecar/nanodet_ncnn_sidecar.py
```
По UART можно **одновременно** держать Betaflight Configurator открытым по USB — это
удобно для следующего шага.

</details>

---

### Шаг 2 — Проверка БЕЗ пропеллеров (обязательно)

> 🚨 **Сними пропеллеры.** Этот шаг — чтобы убедиться, что дрон поворачивается в
> **правильную** сторону. Перепутанный знак = дрон поедет ОТ человека/в стену.

1. Pi запущен (раздел A4/B4), открой `http://<ip-пи>:8080` с ноутбука/телефона в той же сети.
2. Подай питание на дрон (**пропы сняты!**), включи пульт, **заармь** как обычно.
3. Включи тумблер **MSP Override**.
4. На веб-панели нажми **пробел** (бейдж станет «LOCK ON») и пройди перед камерой —
   на тебе появится рамка. Если людей несколько — **кликни мышкой по себе**.
5. Смотри направления (по Receiver-вкладке Configurator на UART, либо по тому, как
   полётник пытается крутить моторы):
   - человек **справа** в кадре → дрон должен поворачивать **вправо** (по часовой). Не туда → `FC_INVERT_YAW=1`.
   - человек **далеко/мелкий** → команда **вперёд**. Едет назад → `FC_INVERT_PITCH=1`.
6. Сбрось тумблер MSP Override, разармь, выключи. Перезапусти Pi с нужными `FC_INVERT_*`.
...
выбери человека кликом. Дрон начнёт доворачивать и держать дистанцию. **Чуть что —
сбрось тумблер** (управление мгновенно вернётся к тебе). Начинай с малых значений
`FC_MAX_YAW`/`FC_MAX_PITCH`.

### Тонкая настройка (переменные окружения)
...
| `FC_PORT` / `FC_BAUD` / `FC_RATE` | порт / скорость / частота отправки RC (Гц) | `/dev/ttyAMA0` / `115200` / `50` |

Полный список — также в [`tools/nanodet-sidecar/README.md`](tools/nanodet-sidecar/README.md#follow-me-flight-betaflight-fc--lock-on-without-a-gimbal).
...
| `cannot open FC serial port` | неверный `FC_PORT` (`ls /dev/ttyACM*` или `/dev/serial0`), нет прав (`usermod -aG dialout`), кабель |
| Pi не видит порт по USB | другой кабель/порт; `dmesg \| tail`; на UART — включён ли serial в `raspi-config` |
| Дрон не реагирует | не включён тумблер **MSP Override**; маска `msp_override_channels_mask` не сохранена; в **Ports** не включён MSP (UART-вариант) |
...
| Не подключается Configurator пока работает Pi (USB) | по USB MSP-клиент один: закрой Pi или настраивай по UART |
