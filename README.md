...
5. Смотри по моторам (пропы сняты!): человек **справа** в кадре → дрон должен пытаться
   ...
   перезапусти Pi, добавив `FC_INVERT_YAW=1` и/или `FC_INVERT_PITCH=1`. (Хочешь видеть
   точные значения каналов в Receiver-вкладке Configurator — это умеет вторая
   инструкция, по UART.)
...
#### A7. Тонкая настройка (переменные окружения)
Добавляй их в ту же команду запуска (`A4`):
...
| `FC_RATE` | частота отправки RC, Гц | `50` |

#### A8. Если что-то не так
