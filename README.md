...
> человека. Сначала проверь **со снятыми пропеллерами**: в Betaflight (вкладка
> Receiver) убедись, что каналы двигаются в нужную сторону. Летай только на открытой
> площадке, вдали от людей, под свою ответственность и по местным правилам. Держи
> пульт включённым, **MSP Override — на тумблере AUX** (сбросил тумблер — мгновенно
> вернул себе yaw+pitch). При потере цели/выключенном слежении сайдкар шлёт
> **центр стиков** (дрон висит, не «уезжает»); если процесс на Pi умрёт — поток MSP
> прекратится и сработает failsafe.

### 1. Что нужно
- Дрон с полётником на **Betaflight** и обычным приёмником (CRSF/SBUS) — пульт остаётся.
- **Raspberry Pi 5** + камера (USB или CSI `rpicam`), `pip3 install pyserial`.
- **COCO-модель** детектора (чтобы класс `person` существовал) и `YOLO_FILTER=person`.

### 2. Распайка RPi 5 ↔ полётник

Логика UART у Pi и Betaflight — **3.3 В**, совместимы напрямую. Сигнальные TX↔RX
**крест-накрест**, обязательно общий **GND**. 5 В между платами по сигналу **не**
соединять.

**Вариант A — UART, пайка (рекомендуется для компактности):**

| RPi 5 (40-pin) | Пин | → | Betaflight (свободный UART) |
|---|---|---|---|
| GPIO14 / **TXD0** | физ. **8** | → | **RX** этого UART |
| GPIO15 / **RXD0** | физ. **10** | → | **TX** этого UART |
| **GND** | физ. **6** | → | **GND** |

Питание Pi 5 — от **отдельного БП 5 В/5 А** (BEC полётника Pi 5 обычно не вытянет).
Если очень нужно от платы — только мощный 5 В BEC на пин **2/4 (5V)** + общий GND.

Включить UART на Pi: `sudo raspi-config` → *Interface Options* → *Serial Port* →
«login shell over serial?» **No** → «serial hardware enabled?» **Yes** → reboot.
Порт будет `/dev/ttyAMA0` (он же `/dev/serial0`). Запускать → `FC_PORT=/dev/ttyAMA0`.

**Вариант B — USB, без пайки (проще всего):** воткни USB-кабель Pi → USB полётника.
MSP по USB-VCP включён всегда, порт обычно `/dev/ttyACM0` → `FC_PORT=/dev/ttyACM0`.

### 3. Настройка Betaflight (один раз)
1. **Ports**: на том UART, что распаял (для USB — ничего не нужно), включи **MSP**, 115200.
   Приёмник **не** переводи в MSP — оставь свой CRSF/SBUS.
2. **Modes**: добавь режим **MSP OVERRIDE** и повесь на свободный **тумблер AUX**.
3. **CLI** — разреши перехват только roll+pitch+yaw (бит0 roll=1, бит1 pitch=2,
   бит3 yaw=8 → маска **11**; газ/арм НЕ включаем):
   ```
   set msp_override_channels_mask = 11
   save
   ```
   (в старых версиях параметр называется `msp_override_channels`.)
4. Проверь **failsafe** и что порядок каналов — `map` по умолчанию **AETR**
   (тогда `FC_CH_*` менять не надо; иначе выставь индексы каналов через env).
...
ND_PARAM=nanodet.param ND_BIN=nanodet.bin ND_INPUT=320 \
  ...
  python3 tools/nanodet-sidecar/nanodet_ncnn_sidecar.py
# открой http://<ip-пи>:8080 · пробел = lock-on · клик по человеку = выбрать цель · C = сброс
```
Дрон висит в **angle/horizon**, ты в воздухе включаешь тумблер **MSP Override** и
жмёшь пробел — дальше Pi ведёт цель. Сначала **со снятыми пропами** проверь
направления: если дрон крутит yaw/едет pitch не туда — `FC_INVERT_YAW=1` /
`FC_INVERT_PITCH=1`. Дистанцию задаёт `FC_TARGET_FILL` (доля высоты кадра, которую
должен занимать человек), резвость — `FC_KP_YAW`/`FC_KP_PITCH` и лимиты
`FC_MAX_YAW`/`FC_MAX_PITCH`. Полный список переменных — в
...
> проверялся** — отнесись как к прошивке-черновику и тестируй со снятыми пропеллерами.
