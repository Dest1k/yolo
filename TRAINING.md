...
> 💡 **Проверить сайдкар ещё до обучения** — в каждой папке сайдкара есть
> `get_model.py`, который берёт **готовую COCO-модель**:
> ```bash
> python tools/yolo-fastestv2-sidecar/get_model.py   # мгновенно: готовый ncnn
> python tools/mediapipe-sidecar/get_model.py        # мгновенно: официальный tflite
> ```
> Он печатает готовую строку запуска — гоняешь поток/панель/подвес на известной
> модели, убеждаешься, что пайплайн жив, и только потом обучаешь своё. (Для picodet
> скрипт конвертит из Paddle, для nanodet — из чекпойнта; rknn — не про Pi 5.)
